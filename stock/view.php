...
if (!empty($search)) {
    $where .= " AND d.drug_name LIKE '%$search%'";
}

if (!empty($_GET['category'])) {
    $catId = intval($_GET['category']);
    $where .= " AND d.category = $catId";
}
...
$sql = "
    SELECT 
        ...
        CASE 
            WHEN ps.quantity <= d.minimum_stock_level THEN 1 
            ELSE 0 
        END AS low_stock
    FROM pharmacy_stock ps
    JOIN drugs d ON d.id = ps.drug
    ...
    $where
    ORDER BY low_stock DESC, d.drug_name ASC
    LIMIT $limit OFFSET $offset
";

$drugs = $db->query($sql);
if (!$drugs) echo $db->error;
...
<div class="card">
    <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;">
        <h2>Pharmacy Drug Inventory</h2>
        <a href="index.php" class="btn btn-primary">+ Stock Drug</a>
    </div>
    ...
    <form action="" method="GET" class="search-form">
        <div class="search-filters">
            ...
            <div class="filter-group" style="margin-left:16px;">
                <label for="category" style="display:block; margin-bottom:8px; font-weight:600; color:#333;">
                    Category
                </label>
                <select name="category" id="category"
                        style="padding:10px 12px; border:1px solid #ddd; border-radius:6px; font-size:14px; width:200px; background-color:white;">
                    <option value="">All Categories</option>
                    <?php
                    $catSql = $db->query("SELECT id, name FROM categories WHERE status=1 ORDER BY name ASC");
                    while ($cat = $catSql->fetch_assoc()):
            ...
            <div class="filter-group" style="flex-grow: 1;">
                ...
                <div class="search-input-wrapper" style="display:flex;">
                    <input type="text"
                           ...
                           style="flex-grow:1; padding:10px 12px; border:1px solid #ddd; border-radius:6px 0 0 6px; border-right:none; font-size:14px;">
                    <button type="submit"
                            class="search-btn"
...
<div class="table-responsive">
        <table class="table">
          <thead>
            ...
            <tbody>
                <?php $i = $offset + 1; while ($row = $drugs->fetch_assoc()): ?>
                <tr class="<?= $row['low_stock'] ? 'low-stock' : '' ?>">
                    <td><?= $i++ ?></td>
...
function deleteDrug(id) {
    swal("Are you sure?", "This drug will be deleted.", "warning", {
